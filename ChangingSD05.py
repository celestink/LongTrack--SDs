...
a=9.2
b=7.8
PLoad=1360.700002 #Maximum wheel load pressure in MPa
P=PLoad*2.0/3.0
for SDi in range(6): 
#SDi define incremental standard deviation index of tie Young's moduli
	...
	#Starting longitudinal distance for crossties
	l0=st/2.0
	f = open('tieModuli_SD_Incr_'+str(SDi)+'.txt', 'w')
	nrepeat=5
	for Nrand in range(nrepeat): #Repeat each realization nrepeat times
		#Generating random ties Young's Modulus and poisson ratio for concrete ties
		stE0=20.0
		muE0=150.0
		muE=muE0 #mean of Young's modulus,
		stE=SDi*stE0  #Standard deviation  of Young's modulus,
		muPr=0.18
		stPr=0.05
		Pr=0.15
		...
		for j in range(nl):
			...
			partitionp2=myrailPart.PartitionFaceByDatumPlane(datumPlane=myrailPart.datums[4],
				faces=myrailPart.faces.findAt(((wr/4, hr/2,	dlp[v][1]+ll/2), ), ))
			...
			myModel.SurfaceTraction(createStepName='Step-2', 
				...
				Region( side1Faces=RailInst.faces.findAt(((wr/4.0, hr/2.0, (dlp[v][0]+ll/2.0)), ), )),
			...
			myModel.EncastreBC(createStepName='Initial', name='BC-dummy'+str(i),
			...
			#CREATE THE TIE CONCRETE  MATERIALS 
				...
				myModel.Tie(adjust=ON, constraintEnforcement=NODE_TO_SURFACE, 
					master=Region(side1Faces=RailInst.faces.findAt(((wr/4.0, -hr/2.0, st/4.0), ), )), 
					name='Constraint-1'+str(i), positionToleranceMethod=COMPUTED,
					slave=Region(side1Faces=mytieInst.faces.findAt(((-3*Lt/8.0, -hr/2.0, (Tr+wt/2.0)), ), )),
					thickness=ON, tieRotations=ON)
					
				##Mesh each tie part
				mytiePart.setElementType(elemTypes=(ElemType(elemCode=C3D20R, elemLibrary=STANDARD),
					ElemType(elemCode=C3D15, elemLibrary=STANDARD), ElemType(elemCode=C3D10,
					elemLibrary=STANDARD)), regions=(mytiePart.cells.findAt(((Lt/4.0, (-hr-ht)/2.0, wt/2.0), ), ), ))
